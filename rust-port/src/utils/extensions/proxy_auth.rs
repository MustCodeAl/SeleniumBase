...
pub fn get_proxy_auth_extension(username: &str, password: &str) -> Result<PathBuf, crate::error::SeleniumBaseError> {
  ...
  "background": {{
    ...
    let js = format!(r#"chrome.webRequest.onAuthRequired.addListener(
  function(details) {{
  ...
  ['blocking']
    ...
    std::fs::write(&bg, js).map_err(|e| crate::error::SeleniumBaseError::InvalidConfig(format!("failed to write background.js: {}", e)))?;
    Ok(dir)
}
