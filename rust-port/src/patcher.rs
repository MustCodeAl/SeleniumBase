...
pub fn patch_chromedriver<P: AsRef<Path>>(path: P) -> Result<(), SeleniumBaseError> {
    let mut content = fs::read(path.as_ref()).map_err(|e| SeleniumBaseError::InvalidConfig(format!("Failed to read chromedriver: {}", e)))?;
    ...
    content = re1.replace_all(&content, |caps: &regex::bytes::Captures| {
        vec![b' '; caps[0].len()]
    }).into_owned();

    let re2 = Regex::new(r"window\.cdc_[a-zA-Z0-9]{22}_(Array|Promise|Symbol|Object|Proxy|JSON|Window)\s*\|\|").unwrap();
    content = re2.replace_all(&content, |caps: &regex::bytes::Captures| {
        vec![b' '; caps[0].len()]
    }).into_owned();

    let re3 = Regex::new(r"'\$cdc_[a-zA-Z0-9]{22}_';").unwrap();
    content = re3.replace_all(&content, |caps: &regex::bytes::Captures| {
    ...
    content = re4.replace_all(&content, |caps: &regex::bytes::Captures| {
    ...
    fs::write(path.as_ref(), content).map_err(|e| SeleniumBaseError::InvalidConfig(format!("Failed to write patched chromedriver: {}", e)))?;
    Ok(())
}
